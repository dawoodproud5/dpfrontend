...
const SignupForm = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        await axios.post(`${url}/api/register/with/email`, obj).then(res => {
            sessionStorage.setItem('token', token);
            router.push('/')
            toast.success("You're logged in but not registered as a client in our database.")
            console.log("submitted for the entred data", res)
        }).catch(err => {
            toast.error("Something went wrong You're not logged in. Please try again.")
        })
    ...
    if (session) {
        return (
            <form className="flex flex-col items-center max-w-[100%] xl:max-w-[100%] space-y-1 sm:space-y-2 mt-8 xl:mt-20 font-Factor_A">
                {/* signup button  */}
                <button
